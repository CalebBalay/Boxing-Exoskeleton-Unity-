...
namespace BetterJoyForCemu {
    public partial class MainForm : Form {
        public bool allowCalibration = false; //Boolean.Parse(ConfigurationManager.AppSettings["AllowCalibration"]);
        ...
        public bool shakeInputEnabled = false; //Boolean.Parse(ConfigurationManager.AppSettings["EnableShakeInput"]);
        public float shakeSesitivity = 10; //float.Parse(ConfigurationManager.AppSettings["ShakeInputSensitivity"]);
        public float shakeDelay = 200; //float.Parse(ConfigurationManager.AppSettings["ShakeInputDelay"]);
        ...
        private void MainForm_Load(object sender, EventArgs e) {
            UnityEngine.Debug.Log("MainForm Load");
        ...
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e) {
            try {
                UnityEngine.Debug.Log("MainForm Closing");
                Program.Stop();
                Environment.Exit(0);
            } catch { }
        }
        ...
        bool doNotRejoin = false; //Boolean.Parse(ConfigurationManager.AppSettings["DoNotRejoinJoycons"]);
            ...
            if (button.Tag.GetType() == typeof(Joycon)) {
                ...
                if (v.other == null && !v.isPro) { // needs connecting to other joycon (so messy omg)
                    ...
                    if (succ)
                        foreach (Button b in con)
                            if (b.Tag == v)
                                UnityEngine.Debug.Log("succ"); //b.BackgroundImage = v.isLeft ? Properties.Resources.jc_left : Properties.Resources.jc_right;
