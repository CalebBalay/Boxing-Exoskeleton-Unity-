...
namespace BetterJoyForCemu {
    public class Joycon {
        ...
        public DebugType debug_type = (DebugType)0; //int.Parse(ConfigurationManager.AppSettings["DebugType"]);
        ...
        int lowFreq = 40; //Int32.Parse(ConfigurationManager.AppSettings["LowFreqRumble"]);
        int highFreq = 120; //Int32.Parse(ConfigurationManager.AppSettings["HighFreqRumble"]);

        bool toRumble = true; //Boolean.Parse(ConfigurationManager.AppSettings["EnableRumble"]);

        bool showAsXInput = false; //Boolean.Parse(ConfigurationManager.AppSettings["ShowAsXInput"]);
        bool showAsDS4 = false; //Boolean.Parse(ConfigurationManager.AppSettings["ShowAsDS4"]);

        public MainForm form;

        public ManageJoycons manager;
        ...
        static float AHRS_beta = 0.05f; //float.Parse(ConfigurationManager.AppSettings["AHRS_beta"]);
        public MadgwickAHRS AHRS = new MadgwickAHRS(0.005f, AHRS_beta); // for getting filtered Euler angles of rotation; 5ms sampling rate
        ...
        public int Attach() {
            ...
            if (isUSB) {
                ...
                UnityEngine.Debug.Log("Using USB.\r\n"); //form.AppendTextBox("Using USB.\r\n");
                ...
                if (a[0] != 0x81) { // can occur when USB connection isn't closed properly
                    UnityEngine.Debug.Log("Resetting USB connection.\r\n"); //form.AppendTextBox("Resetting USB connection.\r\n");
        ...
        private void BatteryChanged() { // battery changed level
            ...
            if (battery <= 1) {
                ...
                UnityEngine.Debug.Log("low battery notification!"); //form.notifyIcon.ShowBalloonTip(0);
            }
        }
        ...
        private int ReceiveRaw() {
            ...
            if (ret > 0) {
                ...
                if (ts_en == raw_buf[1] && !(isSnes || is64)) {
                    UnityEngine.Debug.Log("Duplicate timestamp enqueued.\r\n"); //form.AppendTextBox("Duplicate timestamp enqueued.\r\n");
        ...
        bool dragToggle = false; //Boolean.Parse(ConfigurationManager.AppSettings["DragToggle"]);
        ...
        bool HomeLongPowerOff = true; //Boolean.Parse(ConfigurationManager.AppSettings["HomeLongPowerOff"]);
        long PowerOffInactivityMins = -1; //Int32.Parse(ConfigurationManager.AppSettings["PowerOffInactivity"]);

        bool ChangeOrientationDoubleClick = true; //Boolean.Parse(ConfigurationManager.AppSettings["ChangeOrientationDoubleClick"]);
        long lastDoubleClick = -1;

        string extraGyroFeature = "none"; //ConfigurationManager.AppSettings["GyroToJoyOrMouse"];
        bool UseFilteredIMU = true; //Boolean.Parse(ConfigurationManager.AppSettings["UseFilteredIMU"]);
        int GyroMouseSensitivityX = 1200; //Int32.Parse(ConfigurationManager.AppSettings["GyroMouseSensitivityX"]);
        int GyroMouseSensitivityY = 800; //Int32.Parse(ConfigurationManager.AppSettings["GyroMouseSensitivityY"]);
        float GyroStickSensitivityX = 40.0f; //float.Parse(ConfigurationManager.AppSettings["GyroStickSensitivityX"]);
        float GyroStickSensitivityY = 10.0f; //float.Parse(ConfigurationManager.AppSettings["GyroStickSensitivityY"]);
        float GyroStickReduction = 1.5f; //float.Parse(ConfigurationManager.AppSettings["GyroStickReduction"]);
        bool GyroHoldToggle = true; //Boolean.Parse(ConfigurationManager.AppSettings["GyroHoldToggle"]);
        bool GyroAnalogSliders = false; //Boolean.Parse(ConfigurationManager.AppSettings["GyroAnalogSliders"]);
        int GyroAnalogSensitivity = 400; //Int32.Parse(ConfigurationManager.AppSettings["GyroAnalogSensitivity"]);
        ...
        bool swapAB = false; //Boolean.Parse(ConfigurationManager.AppSettings["SwapAB"]);
        bool swapXY = false; //Boolean.Parse(ConfigurationManager.AppSettings["SwapXY"]);
        bool realn64Range = true; //Boolean.Parse(ConfigurationManager.AppSettings["N64Range"]);
        float stickScalingFactor = 1.00f; //float.Parse(ConfigurationManager.AppSettings["StickScalingFactor"]);
        float stickScalingFactor2 = 1.00f; //float.Parse(ConfigurationManager.AppSettings["StickScalingFactor2"]);
                        ...
                        switch (i) {
                            case 0:
                                ...
                                if (manager.calibrate) {
                                    manager.xA.Add(acc_r[i]);
                                    manager.xG.Add(gyr_r[i]);
                                }
                                break;
                            case 1:
                                ...
                                if (manager.calibrate) {
                                    manager.yA.Add(acc_r[i]);
                                    manager.yG.Add(gyr_r[i]);
                                }
                                break;
                            case 2:
                                ...
                                if (manager.calibrate) {
                                    manager.zA.Add(acc_r[i]);
                                    manager.zG.Add(gyr_r[i]);
                                }
                                break;
                        }
                    }
        ...
        public void Begin() {
            if (PollThreadObj == null) {
                ...
                UnityEngine.Debug.Log("Starting poll thread.\r\n"); //form.AppendTextBox("Starting poll thread.\r\n");
            } else {
                UnityEngine.Debug.Log("Poll cannot start.\r\n"); //form.AppendTextBox("Poll cannot start.\r\n");
            }
        }
        ...
        private void dump_calibration_data() {
            if (isSnes || is64 || thirdParty) {
                short[] temp = (short[])"16384,16384,16384".Split(',').Select(s => short.Parse(s)).ToArray(); //(short[])ConfigurationManager.AppSettings["acc_sensiti"].Split(',').Select(s => short.Parse(s)).ToArray();
                acc_sensiti[0] = temp[0]; acc_sensiti[1] = temp[1]; acc_sensiti[2] = temp[2];
                temp = (short[])"18642,18642,18642".Split(',').Select(s => short.Parse(s)).ToArray(); //(short[])ConfigurationManager.AppSettings["gyr_sensiti"].Split(',').Select(s => short.Parse(s)).ToArray();
                gyr_sensiti[0] = temp[0]; gyr_sensiti[1] = temp[1]; gyr_sensiti[2] = temp[2];
                ushort[] temp2 = (ushort[])"0x780,0x780,0x780,0x830,0x780,0x780".Split(',').Select(s => ushort.Parse(s.Substring(2), System.Globalization.NumberStyles.HexNumber)).ToArray(); //(ushort[])ConfigurationManager.AppSettings["stick_cal"].Split(',').Select(s => ushort.Parse(s.Substring(2), System.Globalization.NumberStyles.HexNumber)).ToArray();
                stick_cal[0] = temp2[0]; stick_cal[1] = temp2[1]; stick_cal[2] = temp2[2];
                stick_cal[3] = temp2[3]; stick_cal[4] = temp2[4]; stick_cal[5] = temp2[5];
                deadzone = 200; //ushort.Parse(ConfigurationManager.AppSettings["deadzone"]);
                temp2 = (ushort[])"0x780,0x780,0x780,0x830,0x780,0x780".Split(',').Select(s => ushort.Parse(s.Substring(2), System.Globalization.NumberStyles.HexNumber)).ToArray(); //(ushort[])ConfigurationManager.AppSettings["stick2_cal"].Split(',').Select(s => ushort.Parse(s.Substring(2), System.Globalization.NumberStyles.HexNumber)).ToArray();
                ...
                deadzone2 = 200; //ushort.Parse(ConfigurationManager.AppSettings["deadzone2"]);
                return;
            }
            ...
            for (int i = 0; i < 9; ++i) {
                if (buf_[i] != 0xff) {
                    UnityEngine.Debug.Log("Using user stick calibration data.\r\n"); //form.AppendTextBox("Using user stick calibration data.\r\n");
                    found = true;
                    break;
                }
            }
            if (!found) {
                UnityEngine.Debug.Log("Using factory stick calibration data.\r\n"); //form.AppendTextBox("Using factory stick calibration data.\r\n");
        ...
        private static float[] Getn64StickValues(Joycon input)
        {
            ...
            UnityEngine.Debug.Log(desc); //Debug.WriteLine(desc);
            #endif
